...
def _normalise_property_review(prop: dict, owner: dict | None = None, verification: dict | None = None) -> dict:
    ...
    for key, label, fields in PROPERTY_REVIEW_ITEMS:
        if key == "photos":
            ...
            auto_ready = bool(prop.get("broker_id") or owner.get("broker_id")) and bool(prop.get("rm_id") or owner.get("rm_id"))
            details = f"Broker: {prop.get('broker_id') or owner.get('broker_id') or '-'} / RM: {prop.get('rm_id') or owner.get('rm_id') or '-'}"
        elif key == "amenities":
...
async def _enrich_bookings(db, bookings: list[dict]) -> list[dict]:
    ...
    users = await db.users.find({"user_id": {"$in": list(set(guest_ids + host_ids + broker_ids))}}, {"_id": 0, "password_hash": 0}).to_list(length=len(set(guest_ids + host_ids + broker_ids)) or 1)
    user_map = {u["user_id"]: u for u in users}
    for booking in bookings:
        prop = prop_map.get(booking.get("property_id"), {})
        host = user_map.get(booking.get("host_id"), {})
        broker_id = prop.get("broker_id") or host.get("broker_id")
