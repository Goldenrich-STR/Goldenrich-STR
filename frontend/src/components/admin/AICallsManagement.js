  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-6 rounded-2xl border border-gray-100 shadow-sm gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-charcoal">AI Calling Concierge</h2>
      ...
      <div className="flex space-x-2 border-b border-gray-100">
        <button
          ...
          className={`flex items-center space-x-2 px-4 py-3 font-semibold transition-all ${
            activeSubTab === 'logs'
              ? 'text-terracotta border-b-2 border-terracotta font-bold tracking-tight'
      ...
      {activeSubTab === 'logs' ? (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-stone border-b border-gray-100">
                    <th className="px-6 py-4 text-xs font-bold tracking-tight text-charcoal uppercase tracking-widest">Booking ID</th>
                    <th className="px-6 py-4 text-xs font-bold tracking-tight text-charcoal uppercase tracking-widest">Recipient</th>
                    <th className="px-6 py-4 text-xs font-bold tracking-tight text-charcoal uppercase tracking-widest">Agent Name</th>
                    <th className="px-6 py-4 text-xs font-bold tracking-tight text-charcoal uppercase tracking-widest">Phone</th>
                    <th className="px-6 py-4 text-xs font-bold tracking-tight text-charcoal uppercase tracking-widest">Duration</th>
                    <th className="px-6 py-4 text-xs font-bold tracking-tight text-charcoal uppercase tracking-widest">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-sand-100">
                  {filteredCalls.map((call) => (
                    <tr key={call.call_id} className="hover:bg-stone/50 transition-colors">
                      <td className="px-6 py-4">
                        <span className="font-bold tracking-tight text-charcoal">{call.booking_id}</span>
                      </td>
                      <td className="px-6 py-4">
                        <div>
                          <div className="font-bold text-charcoal">{call.recipient_name}</div>
                          <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold tracking-tight uppercase mt-0.5 ${
                      ...
                      <td className="px-6 py-4">
                        <button
                          ...
                          className="inline-flex items-center text-xs font-bold tracking-tight uppercase tracking-wider text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 px-3 py-1.5 rounded-full transition-all cursor-pointer shadow-sm"
                        >
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-6">
          <div className="flex justify-between items-center">
            <div>
              <h3 className="text-xl font-bold tracking-tight text-charcoal">Voice Calling Agents</h3>
              ...
              {agents.map((agent) => (
                <div 
                  ...
                  className={`border-2 rounded-3xl p-6 transition-all flex flex-col justify-between h-[300px] relative overflow-hidden ${
                    agent.is_active 
                      ? 'border-terracotta bg-terracotta/5 shadow-subtle' 
                      : 'border-gray-100 bg-white hover:border-gray-200 hover:shadow-sm'
                  }`}
                >
                  {agent.is_active && (
                    <div className="absolute top-0 right-0 bg-terracotta text-white text-[9px] font-bold tracking-tight uppercase tracking-wider px-3 py-1 rounded-bl-xl shadow">
                      Active
                    </div>
                  )}
                  ...
                  <div>
                    <div className="flex items-center gap-2.5 mb-4">
                      <div className="p-2.5 bg-gray-50 rounded-2xl text-terracotta">
                        <Volume2 className="w-6 h-6" />
                      </div>
                      <div>
                        <h4 className="font-bold tracking-tight text-lg text-charcoal leading-tight">{agent.agent_name}</h4>
                        <span className="text-[10px] font-bold tracking-tight uppercase tracking-wider text-charcoal-light block mt-0.5">
                          {agent.external_voice_name ? `External (${agent.external_voice_name})` : agent.voice_type} • {agent.language}
                        </span>
                      </div>
                    </div>

                    <div className="space-y-2 mb-4 bg-stone p-4 rounded-2xl border border-gray-100">
                      <span className="block text-[9px] font-bold tracking-tight uppercase tracking-wider text-charcoal-muted">Greeting Greeting</span>
                      <p className="text-xs font-semibold text-charcoal italic line-clamp-3">"{agent.greeting_message}"</p>
                    </div>
                  </div>

                  <div className="flex items-center justify-between gap-3 border-t border-gray-100/60 pt-4 mt-auto">
                    {!agent.is_active ? (
                      <button
                        onClick={() => handleActivateAgent(agent.agent_id)}
                        className="flex-1 py-2 px-3 rounded-xl bg-charcoal hover:bg-neutral-800 text-white font-bold tracking-tight text-xs uppercase tracking-wider transition cursor-pointer text-center"
                      >
                        Set Active
                      </button>
                    ) : (
                      <div className="flex-1 inline-flex items-center justify-center text-green-700 bg-green-50 border border-green-200 py-2 rounded-xl text-xs font-bold tracking-tight uppercase">
        ...
        <div className="fixed inset-0 bg-charcoal/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-8 max-w-xl w-full shadow-premium animate-slide-up relative overflow-hidden">
            ...
            <div className="flex justify-between items-center mb-8 relative z-10">
              <div>
                <h3 className="text-2xl font-bold tracking-tight text-charcoal tracking-tight">New AI Calling Agent</h3>
                <p className="text-sm text-charcoal-muted mt-1">Configure voice profile and greetings below.</p>
              </div>
              <button 
                onClick={() => setShowAgentModal(false)} 
                className="w-10 h-10 bg-gray-50 text-charcoal rounded-full flex items-center justify-center hover:bg-sand-200 transition"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleCreateAgent} className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6 relative z-10">
              <div className="space-y-2">
                <label className="text-xs font-bold tracking-tight text-charcoal-muted uppercase tracking-widest block mb-1">Agent Name</label>
                <input 
                  type="text" 
                  required
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold tracking-tight text-charcoal-muted uppercase tracking-widest block mb-1">Voice Profile</label>
                <select 
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold tracking-tight text-charcoal-muted uppercase tracking-widest block mb-1">Language</label>
                <select 
                  value={formData.language}
                  onChange={e => setFormData({...formData, language: e.target.value})}
                  className="w-full border-2 border-gray-100 rounded-xl px-4 py-2 focus:border-terracotta outline-none transition font-bold"
                >
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold tracking-tight text-charcoal-muted uppercase tracking-widest block mb-1">External Voice (Optional)</label>
                <select 
              ...
              <div className="space-y-2 md:col-span-2">
                <label className="text-xs font-bold tracking-tight text-charcoal-muted uppercase tracking-widest block mb-1">Greeting Prefix</label>
                <input 
                  type="text" 
                  required
              ...
              <div className="md:col-span-2 flex space-x-4 pt-6 border-t border-gray-100 mt-4">
                <button 
                  type="button" 
                  onClick={() => setShowAgentModal(false)} 
                  className="flex-1 py-4 font-bold text-charcoal-muted bg-stone hover:bg-gray-50 rounded-xl transition"
                >
                  Cancel
                </button>
                <button 
                  type="submit" 
                  className="flex-1 btn-premium py-4 shadow-premium hover:-translate-y-1 transition-all duration-300"
                >
                  Create Agent
                </button>
              </div>
      ...
      <div className="bg-white rounded-3xl max-w-lg w-full overflow-hidden shadow-elevated border border-gray-100 flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto flex-1 bg-white min-h-[160px] max-h-[300px]">
          ...
          <div className="bg-stone p-4 rounded-2xl border border-gray-100 text-sm font-medium text-charcoal leading-relaxed relative">
            <div className="absolute top-3 left-4 text-xs text-terracotta font-bold tracking-tight uppercase tracking-wider text-[9px] mb-1 font-sans">
              {call.agent_name}
            </div>
