        ...
        <table className="min-w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {rows.map((booking) => {
              ...
              const isCalendarImport = booking.is_calendar_import === true;
              const canConfirm = !isCalendarImport && (!isDirect || paidStates.includes(lower(booking.payment_status)));
              const current = lower(booking.booking_status);
              return <tr key={booking.booking_id}><td className="whitespace-nowrap px-4 py-3"><p className="font-bold text-slate-900">{booking.booking_id}</p><p className="text-xs text-slate-500">{booking.guest_name || booking.guest?.name || booking.guest?.full_name || 'Guest'}</p></td><td className="px-4 py-3"><p className="max-w-56 truncate font-semibold">{booking.property?.title || propertyMap[booking.property_id]?.name || booking.property_id}</p></td><td className="px-4 py-3"><span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-bold">{text(source)}</span></td><td className="whitespace-nowrap px-4 py-3 text-xs">{text(booking.check_in_date)}<br />{text(booking.check_out_date)}</td><td className="px-4 py-3"><span className="capitalize">{text(booking.payment_status).replace(/_/g, ' ')}</span></td><td className="px-4 py-3"><span className={`rounded-full px-2 py-1 text-xs font-bold capitalize ${current === 'confirmed' ? 'bg-emerald-100 text-emerald-700' : current === 'cancelled' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>{text(booking.booking_status).replace(/_/g, ' ')}</span></td><td className="px-4 py-3"><div className="flex justify-end gap-1">{isCalendarImport ? <span className="whitespace-nowrap text-xs font-semibold text-slate-500" title="Cancel or modify this booking on the OTA, then use Sync Now.">Managed on OTA</span> : <>{!['confirmed', 'completed', 'cancelled'].includes(current) && <button aria-label="Confirm reservation" className="grid h-9 w-9 place-items-center rounded-md text-emerald-700 hover:bg-emerald-50 disabled:cursor-not-allowed disabled:opacity-35" disabled={!canConfirm || busyId === booking.booking_id} onClick={() => changeStatus(booking, 'confirmed')} title={canConfirm ? 'Confirm reservation' : 'Verified payment required for direct booking'} type="button"><CheckCircle2 size={17} /></button>}{current !== 'cancelled' && <button aria-label="Cancel reservation" className="grid h-9 w-9 place-items-center rounded-md text-red-600 hover:bg-red-50 disabled:opacity-35" disabled={busyId === booking.booking_id} onClick={() => changeStatus(booking, 'cancelled')} title="Cancel reservation" type="button"><XCircle size={17} /></button>}</>}</div></td></tr>;
            })}
          </tbody>
        </table>
      </div>
