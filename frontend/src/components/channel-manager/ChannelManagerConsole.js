...
import { BookOpen, CalendarDays, CalendarRange, IndianRupee, Link2, RefreshCw, Wifi } from 'lucide-react';
...
const tabs = [
  ...
  ['price-engine', 'Price Engine', IndianRupee],
];

export default function ChannelManagerConsole() {
  const location = useLocation();
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState(location.pathname.endsWith('/price-engine') ? 'price-engine' : 'integrations');
  ...
  useEffect(() => { load(); }, [load]);
  useEffect(() => { if (location.pathname.endsWith('/price-engine')) setActiveTab('price-engine'); }, [location.pathname]);
  const chooseTab = (key) => {
    setActiveTab(key);
    navigate(key === 'price-engine' ? '/admin/channel-manager/price-engine' : '/admin/channel-manager');
  };
