    ...
    <div>
      ...
      <Panel className="mb-4 p-3">
        <div className="mb-3 flex gap-2 overflow-x-auto">
          {statusTabs.map(([id, label]) => <button key={label} onClick={() => setStatusFilter(id)} className={`whitespace-nowrap rounded-lg px-3 py-2 text-sm font-bold ${statusFilter === id ? 'bg-terracotta text-charcoal' : 'bg-slate-100 text-slate-600'}`}>{label}</button>)}
        </div>
...
const BookingRow = ({ booking, onOpen, onStatus }) => (
  <tr>
    ...
    <td className="px-4 py-3"><div className="flex flex-wrap gap-1"><button onClick={() => onOpen(booking)} className="rounded-lg bg-charcoal px-2 py-1 text-xs font-bold text-white">Review</button><button onClick={() => onStatus(booking, { booking_status: 'confirmed', payment_status: 'paid' })} className="rounded-lg bg-emerald-50 px-2 py-1 text-xs font-bold text-emerald-700">Confirm</button><button onClick={() => onStatus(booking, { booking_status: 'cancelled' })} className="rounded-lg bg-red-50 px-2 py-1 text-xs font-bold text-red-700">Cancel</button></div></td>
  </tr>
);
  ...
  return (
    <Panel className="p-4 xl:sticky xl:top-4 xl:max-h-[calc(100vh-2rem)] xl:overflow-y-auto">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div><p className="text-xs font-bold uppercase text-terracotta">Booking Detail</p><h2 className="text-lg font-black">{booking.booking_id}</h2><p className="text-xs text-slate-500">{booking.property?.title || booking.property_id}</p></div>
      ...
      {selected.loading ? <LoadingState /> : selected.error ? <ErrorState message={selected.error} /> : (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => onStatus(booking, { booking_status: 'confirmed', payment_status: 'paid' })} className="inline-flex items-center justify-center gap-1 rounded-lg bg-emerald-600 px-3 py-2 text-xs font-black text-white"><CalendarCheck className="h-4 w-4" /> Confirm Paid</button>
            <button onClick={() => onStatus(booking, { payment_status: 'refunded' })} className="inline-flex items-center justify-center gap-1 rounded-lg bg-amber-50 px-3 py-2 text-xs font-black text-amber-700"><CreditCard className="h-4 w-4" /> Mark Refunded</button>
